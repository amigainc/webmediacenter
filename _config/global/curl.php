...
function url_get_contents($url, $debug = false, $headers = false, $post = false) {
    ...
    if (strpos($url, 'utm_source') !== false)
        $url = substr($url, 0, strpos($url, 'utm_source'));
    ...
    curl_setopt($ch, CURLOPT_USERAGENT, 'Googlebot/2.1 (http://www.googlebot.com/bot.html)'); //random_user_agent());
    // ignore SSL errors
...
function fgc($url, $header) {
    $context = stream_context_create();

    $header = implode("\r\n", $header);

    stream_context_set_option($context, 'http', 'method', 'GET');
    stream_context_set_option($context, 'http', 'header', $header);
    stream_context_set_option($context, 'http', 'user_agent', random_user_agent());
    stream_context_set_option($context, 'http', 'ignore_errors', true);
    stream_context_set_option($context, 'http', 'follow_location', 1);

    $text = @file_get_contents($url, null, $context);

    if (strpos(strtolower($text), 'html') !== false)
        return $text;
...
function random_user_agent() {
    ...
    return 'Accept:text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8 User-Agent:' . $user_agent;
}
